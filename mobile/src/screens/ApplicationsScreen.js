...
// "My Applications" tab — the tab-bar-reachable equivalent of BundleScreen (which stays as a
// separate root-stack screen for the Home "Bundle & conflicts" button). Reads citizenId from
// CitizenContext instead of route.params since a bottom tab has no navigation params to receive.
export default function ApplicationsScreen({ navigation }) {
  ...
  const run = useCallback(() => {
    if (!citizenId) {
      setState({ loading: false, error: null, bundle: null, schemes: [], conflicts: [] });
      return undefined;
    }
    let cancelled = false;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={["top"]}>
      <ScrollView contentContainerStyle={styles.page}>
        <Text style={typography.h1}>My Applications</Text>
        <Text style={[typography.muted, { marginBottom: spacing.md }]}>
          The best conflict-free combination of schemes for your profile.
        </Text>

        {!citizenId && <InfoMessage>Complete your profile first to see your applications.</InfoMessage>}
